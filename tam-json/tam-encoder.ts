...
function chooseBase(
  inv: Inventory,
  override?: number,
): { baseZoom: number; baseDataset?: string; zmax: number } {
...
export function encodeTamJson(
  inv: Inventory,
  opts?: { baseZoom?: number; zmax?: number },
): TamJson {
  ...
  const ensureDs = (name: string) => {
    if (!dsIndex.has(name)) {
      dsIndex.set(name, datasets.length)
      datasets.push(name)
    }
