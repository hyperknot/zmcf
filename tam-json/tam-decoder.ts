...
export class TAM {
  ...
  datasetHere(
    lat: number,
    lon: number,
  ): { zoom: number; datasetIndex: number; dataset: string | Record<string, unknown> | null } {
    ...
    return {
      zoom: this.zbase,
      datasetIndex: base,
      dataset: base >= 0 ? (this.datasets[base] as any) : null,
    }
  }
