...
function parseArgs(argv: string[]) {
  ...
  for (let i = 3; i < args.length; i++) {
    ...
    if (a === "--base-zoom") {
      ...
      const v = Number(args[++i]);
      if (!Number.isFinite(v) || v < 0) { console.error("Invalid --zmax"); process.exit(1); }
      zmaxOverride = v | 0;
    } else {
...
async function main() {
  ...
  console.log(`[tam-json] zbase=${tam.zbase}, zmax=${tam.zmax}, datasets=${tam.datasets.length}, base_dataset=${tam.base_dataset}`);
