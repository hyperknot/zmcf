...
function isGlobalItem(it: JsonItem): boolean {
  ...
  const minLat = -85.0511287798066, maxLat = 85.0511287798066;
 ...
 */
function chooseBase(inv: JsonInventory, override?: number): { baseZoom: number; baseDataset?: string } {
  const items = inv.items || [];
  if (typeof override === "number") {
    const globals = items.filter(isGlobalItem);
    const baseDataset = globals.sort((a,b)=> (b.max_zoom|0)-(a.max_zoom|0))[0]?.name;
 ...
 */
export function buildZMJFromInventory(inv: JsonInventory, baseZoom: number, baseDataset?: string): ZMJJson {
  const rects: ZMJRect[] = [];
...
function main() {
  ...
  let input = "", output = "", baseOverride: number|undefined;
  const rest: string[] = [];
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (a === "--base-zoom") {
      const v = Number(args[++i]);
      if (!Number.isFinite(v) || v < 0) { console.error("Invalid --base-zoom"); process.exit(1); }
      baseOverride = v|0;
    } else {
      rest.push(a);
    }
  ...
  console.log(`[zmj] Base zoom: ${zmj.base_zoom}, base dataset: ${zmj.base_dataset ?? "none"}, rects: ${zmj.rects.length}, wrote ${Buffer.byteLength(data)} bytes -> ${output}`);
}
